...
public class QSPanel extends ViewGroup {
    ...
    private static float TILE_ASPECT;
...
//    private BrightnessController mBrightnessController;
    ...
    private int mFirstLineGap;
    private int mRowGap;
    private int mMaxRow;
    private int mScreenHeight;
    private boolean isLandLabel;
    ...
    public QSPanel(Context context, AttributeSet attrs) {
        ...
        updateDetailText();
		isLandLabel = getResources().getBoolean(R.bool.config_land_qs_label);
        TILE_ASPECT = isLandLabel ? 1.5f : 1.2f;
...
//                (ToggleSlider) findViewById(R.id.brightness_slider));
    ...
    public void updateResources() {
        ...
        mFirstLineGap = res.getDimensionPixelSize(R.dimen.qs_row_first_line_gap);
        mRowGap = res.getDimensionPixelSize(R.dimen.qs_row_gap);
        mMaxRow = Math.max(1,res.getInteger(R.integer.quick_settings_max_rows));
        if (mColumns != columns) {
            mColumns = columns;
            postInvalidate();
        }
...
//        }
    ...
    private void addTile(final QSTile<?> tile) {
        ...
        if(r.tileView instanceof SignalTileView){
            r.tile.setListening(true);
        }else{
            r.tile.setListening(mListening);
        }
...
//        final int brightnessHeight = mBrightnessView.getMeasuredHeight() + mBrightnessPaddingTop;
        ...
        int perH = mScreenHeight/mMaxRow;
        int h = rows == 0 ? brightnessHeight : (getRowTop(rows) + mPanelPaddingBottom + perH);
...
//                mBrightnessPaddingTop + mBrightnessView.getMeasuredHeight());
        ...
        for (TileRecord record : mRecords) {
            ...
            final int top = getRowTop(record.row + 1);
            int right;
    ...
    private int getRowTop(int row) {
    	int height = mScreenHeight / mMaxRow;
    	Log.d(TAG, "getRowTop row = " + row + ", per height = " + height
    			+ ", mCellHeight = " + mCellHeight
    			+ ", mScreenHeight = " + mScreenHeight
    			+ ", mMaxRow = " + mMaxRow, new Exception());
    	return height * (row -1) + (height - mCellHeight);
//        if (row <= 0) return mFirstLineGap;//mBrightnessView.getMeasuredHeight() + mBrightnessPaddingTop;
//        return /*mBrightnessView.getMeasuredHeight() + mBrightnessPaddingTop
//                + */mLargeCellHeight - mDualTileUnderlap + (row - 1) * mCellHeight + mRowGap;
    }
