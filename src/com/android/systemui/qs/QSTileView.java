...
public class QSTileView extends ViewGroup {
    ...
    public QSTileView(Context context) {
        ...
        isLandLabel = res.getBoolean(R.bool.config_land_qs_label);
    ...
    private void updateTopPadding() {
        ...
        mTilePaddingTopPx = 0;//Math.round((1 - largeFactor) * padding + largeFactor * largePadding);
        //calculated by QSPanel.getRowTop;
        requestLayout();
    }
    ...
    private void recreateLabel() {
        ...
        if (mDual) {
            ...
            mLabel.setGravity(isLandLabel ? Gravity.CENTER_VERTICAL : Gravity.CENTER_HORIZONTAL);
            mLabel.setMaxLines(2);
            int leftPadding = isLandLabel ? 15 : 0;
            mLabel.setPadding(leftPadding, 0, 0, 0);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(isLandLabel){
        	labelView().measure(MeasureSpec.makeMeasureSpec(w - iconWidth, MeasureSpec.AT_MOST),
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if(isLandLabel){
        	int left = 0;
        	left += mTileSpacingPx;
        	left += mTilePaddingTopPx;
        	final int iconTop = (h - mIcon.getMeasuredHeight()) / 2;
        	layout(mIcon, left, iconTop);
        	if (mRipple != null) {
        		updateRippleSize(w, h);
        	}
        	left = mIcon.getRight();
//        left += mTilePaddingBelowIconPx;
//        if (mDual) {
//            layout(mDivider, 0, top);
//            left = mDivider.getBottom();
//        }
        	View label = labelView();
        	final int labelTop = (h - label.getMeasuredHeight())/2;
        	layout(label, left, labelTop);
        } else {
        	layout(mTopBackgroundView, 0, mTileSpacingPx);
