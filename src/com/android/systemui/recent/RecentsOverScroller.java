 ...
 */
public class RecentsOverScroller {
    private int mMode;
     ...
     * @param context
     */
    public RecentsOverScroller(Context context) {
        this(context, null);
    }
     ...
     */
    public RecentsOverScroller(Context context, Interpolator interpolator) {
     ...
     * @hide
     */
    public RecentsOverScroller(Context context, Interpolator interpolator, boolean flywheel) {
    ...
    public void fling(int startX, int startY, int velocityX, int velocityY,
            int minX, int maxX, int minY, int maxY, int discreteWidthX, int discreteWidthY) {
        fling(startX, startY, velocityX, velocityY, minX, maxX, minY, maxY, 0, 0, discreteWidthX,
                discreteWidthY);
    }
     ...
     */
    public void fling(int startX, int startY, int velocityX, int velocityY,
            int minX, int maxX, int minY, int maxY, int overX, int overY, int discreteWidthX,
            int discreteWidthY) {
        ...
        mMode = FLING_MODE;
        mScrollerX.fling(startX, velocityX, minX, maxX, overX, discreteWidthX);
        mScrollerY.fling(startY, velocityY, minY, maxY, overY, discreteWidthY);
    }
    ...
    static class SplineOverScroller {
        ...
        boolean springback(int start, int min, int max) {// call from outside
            mFinished = true;
        ...
        void fling(int start, int velocity, int min, int max, int over, int discreteWidth) {
            mOver = over;
            mFinished = false;
            ...
            // ##modules(Systemui recent): add for sliding to center_horizontal
            // ##date: 2014-10.23 author: ruijie.lrj@alibaba-inc.com
            // int discreteWidth = (int)
            // getContext().getResources().getDimensionPixelSize(
            // R.dimen.status_bar_recents_item_width);

            if (discreteWidth > 0 && mFinal >= min && mFinal <= max) {
                int n = (int) mFinal / discreteWidth;
                int delta = mFinal % discreteWidth;
                if (delta > discreteWidth / 2) {
                    // adjustDuration(mStart, mFinal, (n + 1) * childWidth);
                    mFinal = (n + 1) * discreteWidth;
                    mSplineDistance = mFinal - start;
                    mSplineDuration = mDuration;
                } else {
                    // adjustDuration(mStart, mFinal, n * childWidth);
                    mFinal = n * discreteWidth;
                    mSplineDistance = mFinal - start;
                    mSplineDuration = mDuration;
                }
            ...
            switch (mState) {
                case SPLINE:
                    ...
                    if (mDuration < mSplineDuration) {
                        ...
                        mStartTime += mDuration;// rebegin and bounce
                        onEdgeReached();
                    } else {
