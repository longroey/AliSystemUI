...
public class AliProgress extends View implements Animation.AnimationListener {
    ...
    private static final int UPDATEDURATION = 0; // 250ms
    ...
    public AliProgress(Context context, AttributeSet attrs) {
        ...
        // mLightImage = BitmapFactory.decodeResource(context.getResources(),
        // R.drawable.bashboard_white_light_normal);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        //luruicanvas.drawArc(mArcBounds, 360, 360, false, mRimPaint);
		canvas.drawArc(mRectBounds, 360, 360, false, mRimPaint);
        canvas.drawArc(mArcBounds, -90, mDegree, false, mArcPaint);
        /*
         * if (!mIsClearing) { double angle = (mDegree - 90) * Math.PI / 180.0;
         * float left = (float)(mLightRadius + mLightRadius * Math.cos(angle) -
         * mLightImageWidthHalf + mLightBounds); float top =
         * (float)(mLightRadius + mLightRadius * Math.sin(angle) -
         * mLightImageWidthHalf + mLightBounds); canvas.drawBitmap(mLightImage,
         * left + 2, top + 3, null); }
         */
    }
    ...
    private void parseAttributes(TypedArray a) {
        ...
        mLightBounds = 12;//(int) a.getDimension(R.dimen.default_rim_width, 7);
    ...
    private void setupBounds() {
        ...
        mArcBounds.set(boundsWidth + mLightBounds, boundsWidth + mLightBounds,
                getLayoutParams().width - boundsWidth - mLightBounds, getLayoutParams().height
                        - boundsWidth - mLightBounds);
    }*/

	 private void setupBounds() {
        int boundsWidth = mArcWidth;
        mRectBounds.set( mLightBounds,  mLightBounds,
                getLayoutParams().width  - mLightBounds, getLayoutParams().height
                       - mLightBounds);
    ...
    private void setupPaints() {
        ...
        mRimPaint.setStrokeWidth(mLightBounds);
    }
            ...
            if (isClickAccerlateButton) {
                ...
                if (currentMemorySize > memorySize) {
                    ...
                    if (releaseMemorySize > 0) {
                        String textFormat = context.getResources().getString(
                                R.string.recent_task_release_memory);
                        String finalText = String.format(textFormat, releaseMemorySize);
                        Toast.makeText(context, finalText, Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(context, R.string.recent_task_no_more_released,
    ...
    private class AliAnimation extends Animation {
        ...
        protected void applyTransformation(float interpolatedTime, Transformation t) {
            if (mType == CLEARANIMATIONTYPE) {
                // half animation to change progress to zero, another half
                // change from zero to mEndProgress
                ...
                mIsClearing = false;
                setProgress(mBeginProgress - (mProgressChange * (int) (interpolatedTime * 100))
                        / 100);
            }
        }
    }
