 ...
 * allowing it to be larger than the physical display. A HorizontalScrollView is
 * a {@link FrameLayout}, meaning you should place one child in it containing
 * the entire contents to scroll; this child may itself be a layout manager with
 ...
 * require a HorizontalScrollView, but using the two together is possible to
 ...
 * scrolling, use either {@link ScrollView} or {@link ListView}.
 *
 * @attr ref android.R.styleable#HorizontalScrollView_fillViewport
 */
public class HorizontalSuperScrollView extends FrameLayout {
    ...
    private RecentsOverScroller mScroller;
    ...
    /* YUNOS_BEGIN */
    // ##modules(Systemui recent): add for sliding to center_horizontal, maybe
    // you should change its value when config changes;
    ...
    public HorizontalSuperScrollView(Context context) {
        this(context, null);
    }

    public HorizontalSuperScrollView(Context context, AttributeSet attrs) {
        this(context, attrs, com.android.internal.R.attr.horizontalScrollViewStyle);
    }

    public HorizontalSuperScrollView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initScrollView() {
        /* YUNOS_BEGIN */
        // ##modules(Systemui recent): modify for sliding to center_horizontal,
        // use our own scroller;
        // ##date: 2014-10.23 author: ruijie.lrj@alibaba-inc.com
        mScroller = new RecentsOverScroller(getContext());
        /* YUNOS_END */
        setFocusable(true);
    ...
    public void updateItemsNum(int num) {
        mItemsNum = num;
    }

    /* YUNOS_BEGIN */
    // ##modules(Systemui recent): add for sliding to center_horizontal, when
    // there is only one task, this variable is needed;
    // ##date: 2014-10.23 author: ruijie.lrj@alibaba-inc.com
    public boolean mIsRemoveMoveLeft;

    /* YUNOS_END */

    /* YUNOS_BEGIN */
    // ##modules(Systemui recent): add for sliding to center_horizontal, this
    // part of code comes from FrameLayout.java, but I changed it, pay attention
    // to mIsRemoveMoveLeft;
    // ##date: 2014-10.23 author: ruijie.lrj@alibaba-inc.com
    void layoutChildren(int left, int top, int right, int bottom,
            boolean forceLeftGravity) {
            	forceLeftGravity=true;
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                final int absoluteGravity = Gravity.CENTER_HORIZONTAL;//Gravity.getAbsoluteGravity(gravity, layoutDirection);
                ...
                switch (absoluteGravity & Gravity.HORIZONTAL_GRAVITY_MASK) {
                    ...
                    case Gravity.LEFT:
                    default:
                        // int n = ((ViewGroup) child).getChildCount();
                        // childLeft = parentLeft + lp.leftMargin;
                        // when there is only one task, something is different
                        if (mItemsNum == 1 && (mScrollX >= 0 || !mIsRemoveMoveLeft)) {
                            childLeft = parentRight - width - lp.rightMargin;
                            mIsRemoveMoveLeft = false;
                        } else {
                            childLeft = parentLeft + lp.leftMargin;
                            mIsRemoveMoveLeft = true;
                        }
                }
                ...
                switch (verticalGravity) {
                    ...
                    default:
                        childTop = parentTop + lp.topMargin;
								...
								Log.d(TAG,"l="+Integer.valueOf(childLeft)+
								 " t="+Integer.valueOf(childTop)+" h="+Integer.valueOf(height)+
								 " w="+Integer.valueOf(width)+" ga="+Integer.valueOf(absoluteGravity)+
								 " va="+Integer.valueOf(verticalGravity));
								 int adj=0;
								 if(childLeft<0){
								    childLeft=0;
								    adj-=childLeft;
                ...
                //child.layout(childLeft, childTop, childLeft + width, childTop + height);
                 child.layout(childLeft, childTop, adj + width, childTop + height);
            }
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                /* YUNOS_BEGIN */
                // ##modules(Systemui recent): add for sliding to
                // center_horizontal
                // ##date: 2014-10.23 author: ruijie.lrj@alibaba-inc.com
                // int n = ((ViewGroup) getChildAt(0)).getChildCount();
//BUG FIX FOR XIAOYI Remove center alignment
//                if (mScroller.springBack(mScrollX, mScrollY,
//                        mItemsNum == 1 && !mIsRemoveMoveLeft ? 0
//                                : -mItemWidth,
//                        mItemsNum == 1 && mIsRemoveMoveLeft ? getScrollRange() - mItemWidth
//                                : getScrollRange(),
//                        0, 0)) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    Log.d(TAG, "mScrollX="+mScrollX+" mScrollY="+mScrollY+" range="+range+" canOverscroll="+
                            Boolean.valueOf(canOverscroll)); 
                ...
                if (mIsBeingDragged) {
                    ...
                    if (getChildCount() > 0) {
                        if ((Math.abs(initialVelocity) > mMinimumVelocity)) {
                            ...
                            /* YUNOS_BEGIN */
                            // ##modules(Systemui recent): modify for sliding to
                            // center_horizontal
                            // ##date: 2014-10.23 author:
                            // ruijie.lrj@alibaba-inc.com
                            // int n = ((ViewGroup)
                            // getChildAt(0)).getChildCount();
//BUG FIX FOR XIAOYI C20M-17                        	
//                            if (mScroller.springBack(mScrollX, mScrollY, mItemsNum == 1
//                                    && !mIsRemoveMoveLeft ? 0 : -mItemWidth,
//                                    mItemsNum == 1 && mIsRemoveMoveLeft ? getScrollRange()
//                                            - mItemWidth
//                                            : getScrollRange(), 0, 0)) {
//                                /* YUNOS_END */
//                                postInvalidateOnAnimation();
//                            } else {
//                                clampInChildItemWidth();
//                            }
                        }
                    }
    ...
    /* YUNOS_BEGIN */
    // ##modules(Systemui recent): add for sliding to center_horizontal, as
    // there is more than one tasks, when you move this view,then stop and raise
    // your finger, this method make sure this view will stop at the right
    // positon
    // ##date: 2014-10.23 author: ruijie.lrj@alibaba-inc.com
    private void clampInChildItemWidth() {
        int childWidth = mItemWidth;
        int n = (int) Math.abs(mScrollX / childWidth);
        int delta = mScrollX % childWidth;
        if (Math.abs(delta) > childWidth / 2) {
            smoothScrollBy((int) Math.signum(delta) * (childWidth - Math.abs(delta)), 0);
        } else {
    ...
    /* YUNOS_BEGIN */
    // ##modules(Systemui recent): add for sliding to center_horizontal, when
    // this view exceed normal visible range, this method will be called;
    // ##date: 2014-10.23 author: ruijie.lrj@alibaba-inc.com
    ...
    protected boolean overScrollBy(int deltaX, int deltaY,
        ...
        final int left =-10;// mItemsNum == 1 && !mIsRemoveMoveLeft ? -maxOverScrollX : -maxOverScrollX - mItemWidth;
  
        // final int right = maxOverScrollX + scrollRangeX;
        final int right = mItemsNum == 1 && mIsRemoveMoveLeft ? maxOverScrollX - mItemWidth
                : maxOverScrollX + scrollRangeX+10;
        ...
        Log.d(TAG,"mIsRemoveMoveLeft="+Boolean.valueOf(mIsRemoveMoveLeft)+" maxOverScrollX="+
                 Integer.valueOf(maxOverScrollX)+" mItemWidth="+Integer.valueOf(mItemWidth)+
                 " scrollRangeX="+Integer.valueOf(scrollRangeX));
    ...
    protected void onOverScrolled(int scrollX, int scrollY,
        ...
        if (!mScroller.isFinished()) {
            ...
            if (clampedX) {
                ...
                // ##date: 2014-10.23 author: ruijie.lrj@alibaba-inc.com
                // int n = ((ViewGroup) getChildAt(0)).getChildCount();
                mScroller.springBack(mScrollX, mScrollY, mItemsNum == 1 && !mIsRemoveMoveLeft ? 0
                        : -mItemWidth,
                        mItemsNum == 1 && mIsRemoveMoveLeft ? getScrollRange() - mItemWidth
                                : getScrollRange(), 0, 0);
                /* YUNOS_END */
    ...
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        ...
        info.setClassName(HorizontalSuperScrollView.class.getName());
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(HorizontalSuperScrollView.class.getName());
    ...
    private int getScrollRange() {
        ...
        if (getChildCount() > 0) {
            ...
            Log.d(TAG,"child.getWidth="+Integer.valueOf(child.getWidth())+" getWidth()="+Integer.valueOf(getWidth())+
        		    " mPaddingLeft="+Integer.valueOf(mPaddingLeft)+" mPaddingRight="+Integer.valueOf(mPaddingRight));
        }
     ...
     * </p>
     *
     * @param leftFocus look for a candidate is the one at the left of the
     *            bounds if leftFocus is true, or at the right of the bounds if
     *            leftFocus is false
     ...
     * @param left the left offset of the bounds in which a focusable must be
     *            found
     * @param right the right offset of the bounds in which a focusable must be
     *            found
     * @return the next focusable component in the bounds or null if none can be
     *         found
     */
     ...
     *
     * @param direction the scroll direction:
     *            {@link android.view.View#FOCUS_LEFT} to go the left of the
     *            view or {@link android.view.View#FOCUS_RIGHT} to go the right
     * @return true if the key event is consumed by this method, false otherwise
     */
     ...
     *
     * @param direction the scroll direction:
     *            {@link android.view.View#FOCUS_LEFT} to go left
    ...
    public final void smoothScrollBy(int dx, int dy) {
        ...
        if (duration > ANIMATED_SCROLL_GAP) {
            ...
            final int scrollX = mScrollX;
            dx = Math.max(-mItemWidth, Math.min(scrollX + dx, maxX)) - scrollX;
    ...
    public void fling(int velocityX) {
        if (getChildCount() > 0) {
            ...
            mScroller.fling(mScrollX, mScrollY, velocityX, 0, -mItemWidth,
                    Math.max(0, right - width), 0, 0, width / 2, 0, mItemWidth, 0);
    ...
    public void scrollTo(int x, int y) {
        ...
        if (getChildCount() > 0) {
            View child = getChildAt(0);
            // int n = ((ViewGroup) child).getChildCount();
            x = clampX(x, mItemsNum == 1 && !mIsRemoveMoveLeft ? mItemWidth :
                    getWidth() - mPaddingRight - mPaddingLeft,
                    child.getWidth(), mItemWidth);
    ...
    /* YUNOS_BEGIN */
    // ##modules(Systemui recent): add for sliding to center_horizontal, when
    // remove a item view, this will be called;
    // ##date: 2014-10.23 author: ruijie.lrj@alibaba-inc.com
    private static int clampX(int n, int my, int child, int width) {
        // if (my >= child || n < 0) {
        // return 0;
        // }
        if (n <= -width) {
     ...
     * copy method fom FrameLayout 2014/10/13
     *
    ...
    public void setForegroundGravity(int foregroundGravity) {
        mForegroundGravity = getForegroundGravity();
     ...
     * views;Copy method from FrameLayout at 2014/10/13 in the frame layout. Any
     * padding in the Drawable will be taken into account by ensuring that the
     * children are inset to be placed inside of the padding area.
     *
