...
public class RecentTasksLoader implements View.OnTouchListener {
    ...
    static final boolean DEBUG = PhoneStatusBar.DEBUG || true;//lurui
    ...
    private RecentTasksLoader(Context context) {
     ...
     //   int thumbnailHeight = (int) res
		...
		Log.e(TAG,"thumbnailWidth="+Integer.valueOf(thumbnailWidth)+" thumbnailHeight="+Integer.valueOf(thumbnailHeight));

		int color = res.getColor(R.drawable.status_bar_recents_app_thumbnail_background);

        mDefaultThumbnailBackground = new ColorDrawableWithDimensions(color, thumbnailWidth,
                thumbnailHeight);
    }
    ...
    TaskDescription createTaskDescription(int taskId, int persistentTaskId, Intent baseIntent,
        ...
        if (resolveInfo != null) {
            ...
            if (title != null && title.length() > 0) {
                if (DEBUG)
                    Log.v(TAG, "creating activity desc for id=" + persistentTaskId + ", label="
                            + title);

                TaskDescription item = new TaskDescription(taskId, persistentTaskId, resolveInfo,
                        baseIntent, info.packageName, description);
                item.setLabel(title);
    ...
    private Bitmap getRoundedCornerBitmap(Bitmap bitmap) {
        ...
        final float roundPx = mContext.getResources().getDimensionPixelSize(
    ...
    Drawable getFullResDefaultActivityIcon() {
        return getFullResIcon(Resources.getSystem(), com.android.internal.R.mipmap.sym_def_app_icon);
    }
    ...
    /*
     * private Drawable getFullResIcon(ResolveInfo info, PackageManager
     * packageManager) { Resources resources; try { resources =
     * packageManager.getResourcesForApplication(
     * info.activityInfo.applicationInfo); } catch
     * (PackageManager.NameNotFoundException e) { resources = null; } if
     * (resources != null) { int iconId = info.activityInfo.getIconResource();
     * if (iconId != 0) { return getFullResIcon(resources, iconId); } } return
    ...
    private Drawable getFullResIcon(ResolveInfo info, PackageManager packageManager) {
        ...
        Drawable icon = null;
        // fix the Contacts app's activities(phone,sms,contacts)icons
        try {
            if (info.activityInfo.packageName.equals(Constants.ALI_PACKAGE_NAME_CONTACT)) {

                if (info.activityInfo.name.equals(Constants.ALI_PACKAGE_NAME_PHONE_MAIN)) {
                    // com.yunos.alicontacts.activities.DialtactsActivity
                    ComponentName mComponentName = new ComponentName(
                            info.activityInfo.packageName, info.activityInfo.name);
                    icon = packageManager.getActivityIcon(mComponentName);
                    // icon =
                    // packageManager.getApplicationIcon(Constants.ALI_PACKAGE_NAME_PHONE);
                } else if (info.activityInfo.name.equals(Constants.ALI_PACKAGE_NAME_SMS_MAIN)) {
                    // com.yunos.alimms.ui.ConversationList
                    ComponentName mComponentName = new ComponentName(
                            Constants.ALI_PACKAGE_NAME_SMS, Constants.ALI_PACKAGE_NAME_SMS_MAIN);
                    icon = packageManager.getActivityIcon(mComponentName);
                } else if (info.activityInfo.name.equals(Constants.ALI_PACKAGE_NAME_CONTACT_MAIN)) {
                    // com.yunos.alicontacts.activities.PeopleActivity2
                    icon = packageManager.getApplicationIcon(Constants.ALI_PACKAGE_NAME_CONTACT);
                }
            } else if (info.activityInfo.packageName.equals(Constants.ALI_PACKAGE_NAME_INCALLUI)) {
                if (info.activityInfo.name.equals(Constants.ALI_PACKAGE_NAME_INCALLUI_MAIN)) {
                    ComponentName mComponentName = new ComponentName(
                            Constants.ALI_PACKAGE_NAME_CONTACT,
                            Constants.ALI_PACKAGE_NAME_PHONE_MAIN);
                    icon = packageManager.getActivityIcon(mComponentName);
                }
            }

            if (info.activityInfo.packageName.equals(Constants.ALI_PACKAGE_NAME_SECURITYCENTER)) {
            ...
            Log.e(TAG, "fix the Contacts app's activities exception " + e);
        }
        if (icon == null) {
            //modified by harry 20180413 start
            try {
                icon = info.activityInfo.applicationInfo.loadIcon(packageManager);
            } catch (Exception e) {
                Log.e(TAG, "fix the Contacts app's activities exception " + e);
                icon = null;
            }
            //modified by harry 20180413 end
        }
        return icon;
    }
    ...
    public boolean onTouch(View v, MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            Log.d(TAG, "mediatek_ onTouch() begin  ACTION_DOWN preloadRecentTasksList");
        } else if (action == MotionEvent.ACTION_CANCEL) {
            cancelPreloadingRecentTasksList();
            Log.d(TAG, "mediatek_ onTouch() begin  ACTION_CANCEL cancelPreloadingRecentTasksList");
        } else if (action == MotionEvent.ACTION_UP) {
            // Remove the preloader if we haven't called it yet
            mHandler.removeCallbacks(mPreloadTasksRunnable);
            Log.d(TAG, "mediatek_ onTouch() begin  ACTION_UP removeCallbacks");
            if (!v.isPressed()) {
                cancelLoadingThumbnailsAndIcons();
                Log.d(TAG, "mediatek_ onTouch() begin  ACTION_UP cancelLoadingThumbnailsAndIcons");
            }
    ...
    private void cancelLoadingThumbnailsAndIcons() {
        Thread.dumpStack();
        Log.d(TAG, "cancelLoadingThumbnailsAndIcons() mState = " + mState);
        if (DEBUG) {
            Log.d(TAG, "cancelLoadingThumbnailsAndIcons()");
        }
        if (mRecentsPanel != null && mRecentsPanel.isShowing()) {
            if (DEBUG) {
                Log.d(TAG, "cancelLoadingThumbnailsAndIcons() mRecentsPanel.isShowing() return");
            }
            return;
        }
    ...
    public TaskDescription loadFirstTask() {
        ...
        if (recentTasks.size() > 0) {
            ...
            if (Constants.ALI_PACKAGE_NAME_HOMESHELL.equals(intent.getComponent().getPackageName())
                    ||
                    isCurrentHomeActivity(intent.getComponent(), null)) {
                return null;
            }
            ...
            // Don't load InCallUI
            if (Constants.ALI_PACKAGE_NAME_INCALLUI.equals(intent.getComponent().getPackageName())
                    && Constants.ALI_PACKAGE_NAME_INCALLUI_MAIN.equals(intent.getComponent()
                            .getClassName())) {
                return null;
            }

            item = createTaskDescription(recentInfo.id, recentInfo.persistentId,
    ...
    /* YUNOS_BEGIN */
    // ##modules(Systemui recent): add for sliding to center_horizontal, called
    // by phoneStatusbar
    // ##date: 2014-10.23 author: ruijie.lrj@alibaba-inc.com

    public int getTasksNumBeforeShow() {
        if (mLoadedTasks != null) {
            return mLoadedTasks.size();
        } else {
            final ActivityManager am = (ActivityManager) mContext
                    .getSystemService(Context.ACTIVITY_SERVICE);

            final List<ActivityManager.RecentTaskInfo> recentTasks = am.getRecentTasksForUser(5,
                    ActivityManager.RECENT_IGNORE_UNAVAILABLE, UserHandle.CURRENT.getIdentifier());
            TaskDescription item = null;
            final int n = recentTasks.size();
            int m = 0;
            if (n > 0) {
                for (int i = 0; i < n; i++) {
                    ...
                    // Don't load the current home activity.
                    if (Constants.ALI_PACKAGE_NAME_HOMESHELL.equals(intent.getComponent()
                            .getPackageName()) ||
                            isCurrentHomeActivity(intent.getComponent(), null)) {
                        continue;
                    }
                    ...
                    // Don't load InCallUI
                    if (Constants.ALI_PACKAGE_NAME_INCALLUI.equals(intent.getComponent()
                            .getPackageName())
                            && Constants.ALI_PACKAGE_NAME_INCALLUI_MAIN.equals(intent
                                    .getComponent()
                                    .getClassName())) {
                        continue;
                    }
    ...
    public void loadTasksInBackground(final boolean zeroeth) {
        ...
        if (mState != State.CANCELLED) {
            if (DEBUG) {
                Log.d(TAG, "loadTasksInBackground() return mState = " + mState);
            }
            return;
        }
        ...
        mTaskLoader = new AsyncTask<Void, ArrayList<TaskDescription>, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                Log.d(TAG, "mediatek_ doInBackground() begin ");
                ...
                final List<ActivityManager.RecentTaskInfo> recentTasks = am.getRecentTasks(
                        MAX_TASKS, ActivityManager.RECENT_IGNORE_UNAVAILABLE);
                int numTasks = recentTasks.size();
                if (DEBUG) {
                    Log.d(TAG, numTasks + "");
                    Log.d(TAG, "mediatek_ doInBackground() begin  numTasks = " + numTasks);
                }
                ...
                for (int i = first, index = 0; i < numTasks && (index < MAX_TASKS); ++i) {
                    ...
                    if (Constants.ALI_PACKAGE_NAME_HOMESHELL.equals(intent.getComponent()
                            .getPackageName()) ||
                            isCurrentHomeActivity(intent.getComponent(), homeInfo)) {
                        continue;
                    }
                    ...
                    if (item != null) {
                        ...
                        if (firstScreenful && tasks.size() == mNumTasksInFirstScreenful) {
                            ...
                            Log.d(TAG, "mediatek_ doInBackground() firstScreenful  tasks.size() = " + tasks.size());
                ...
                if (!isCancelled()) {
                    publishProgress(tasks);
                    Log.d(TAG, "mediatek_ doInBackground() !isCancelled()  tasks.size() = " + tasks.size());
                    if (firstScreenful) {
                        // always should publish two updates
                        publishProgress(new ArrayList<TaskDescription>());
                        Log.d(TAG, "mediatek_ doInBackground() firstScreenful !isCancelled()  tasks.size() =" + tasks.size());
                    }
                }
                ...
                Log.d(TAG, "mediatek_ doInBackground() end ");
                return null;
            }
        };
        // Catch RejectedExecutionException exception in Android 4.4.2
        // BugID:5360879
        try {
            mTaskLoader.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            if (DEBUG) {
                Log.d(TAG, "loadTasksInBackground() mTaskLoader executeOnExecutor()");
            }
        } catch (final RejectedExecutionException e) {
            Log.d(TAG,
                    "loadTasksInBackground mThumbnailLoader.executeOnExecutor, rejecting execution",
                    e);
        }
    ...
    private void loadThumbnailsAndIconsInBackground(
            ...
            protected void onProgressUpdate(TaskDescription... values) {
                if (!isCancelled()) {
                    ...
                    if (td.isNull()) { // end sentinel
                        mState = State.LOADED;
                        if (DEBUG) {
                            Log.d(TAG, "loadThumbnailsAndIconsInBackground() onProgressUpdate mState = State.LOADED");
                        }
                    } else {
                        if (mRecentsPanel != null) {
        ...
        try {
            ...
            if (DEBUG) {
                Log.d(TAG, "loadThumbnailsAndIconsInBackground() mTaskLoader executeOnExecutor()");
            }
        } catch (final RejectedExecutionException e) {
