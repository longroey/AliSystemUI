...
public class NetworkControllerImpl extends BroadcastReceiver
    ...
    static final boolean DEBUG = false; //Log.isLoggable(TAG, Log.DEBUG);
    // additional diagnostics, but not logspew
    static final boolean CHATTY = false; // Log.isLoggable(TAG + ".Chat", Log.DEBUG);
...
//            Xlog.d(TAG, "old IMSState=" + mIMSState + ", new IMSState=" + imsState);
        ...
        public boolean isEmergencyOnly() {
            ///M: Support SVLTE. @{
            if (SvLteController.isMediatekSVLteDcSupport(mSubscriptionInfo)) {
                Xlog.d(TAG, "isEmergencyOnly(), will return mSvLteController.isEmergencyOnly()");
                return mSvLteController.isEmergencyOnly();
            }
            ///M: Support SVLTE. @}
            Xlog.d(TAG, "isEmergencyOnly(), will return mServiceState.isEmergencyOnly()");
        ...
        private boolean isRoaming() {
            if (isCdma()) {
                ...
                if (mServiceState == null) {
                    if (DEBUG) {
                        Log.d(TAG, "isRoaming: mServiceState is null");
                    }
                    return false;
                }
                /// M: [ALPS02013362] NullPointerException @}
                ...
                if (DEBUG) {
                    Log.d(TAG, "isRoaming: CDMA mode " + iconMode + " index " + iconIndex);
                }
                ...
                if (DEBUG) {
                    Log.d(TAG, "isRoaming: mServiceState = " + mServiceState);
                }
...
//                        + mLastState.networkType);
                mLastState.networkType = mCurrentState.networkType;
            }
            /// M: Support "Network Type on state". @}
...
//            Xlog.d(TAG, "updateVoLTE: DataNetType=" + mDataNetType +
...
//        Xlog.d(TAG, "showAlwaysAskOrInternetCall= " + selectID);
    ...
    private class NetworkControllerExt implements INetworkControllerExt {
        ...
        private boolean mDebug = false; //!FeatureOptionUtils.isUserLoad();
