...
import com.android.systemui.statusbar.auto.AutoSettingPanel;
import com.android.systemui.statusbar.auto.AutoStatusBarView;
import com.android.systemui.statusbar.auto.NavigatorStatusBarView;
...
import com.android.systemui.statusbar.auto.PanelHolder;
import com.android.systemui.statusbar.auto.TalkingStatusBarView;
import com.android.systemui.statusbar.auto.compositecard.AutoConstant;
import com.android.systemui.statusbar.auto.compositecard.AutoNotificationManager;
import com.android.systemui.statusbar.auto.compositecard.NotificationStackFrameLayout;

import com.mediatek.carcorder.CarcorderManager;

public class PhoneStatusBar extends BaseStatusBar
        implements DemoMode, DragDownHelper.DragDownCallback, ActivityStarter, OnUnlockMethodChangedListener,
        TalkingStatusBarView.TalkingBarStateListener{
    ...
    public static final String ACTION_ENABLE_TXZ_WAKEUP = "com.txznet.txz.wakeup.enable";
    public static final String ACTION_DISABLE_TXZ_WAKEUP = "com.txznet.txz.wakeup.disable";
    ...
    int mNaturalBarWidth = -1;
    int mNaturalMiniBarWidth = -1;
    int mIconSize = -1;
    int mIconHPadding = -1;
    Display mDisplay;
    ...
    AutoStatusBarView mStatusBarView;
    private TalkingStatusBarView mTalkingBarView;
    private NavigatorStatusBarView mNavigatorBarView;
    ...
    AutoSettingPanel mAutoSettingPanel;
    // settings
    ...
    final private ContentObserver mHeadsUpObserver = new ContentObserver(mHandler) {
        ...
        public void onChange(boolean selfChange) {
            ...
            // Log.d(TAG, "heads up is " + (mUseHeadsUp ? "enabled" :
            // "disabled"));
    ...
    public static boolean IS_WIDE_SCREEN = ApplicationUtils.SUPPORT_WIDE_SCREEN;
    ...
    private AutoNotificationManager mAutoNotificationManager;
    ...
    private static final int VISIBLE_LOCATIONS = ViewState.LOCATION_FIRST_CARD | ViewState.LOCATION_TOP_STACK_PEEKING
            | ViewState.LOCATION_MAIN_AREA | ViewState.LOCATION_BOTTOM_STACK_PEEKING;
    ...
    private final Runnable mVisibilityReporter = new Runnable() {
        ...
        public void run() {
            ...
            for (int i = 0; i < N; i++) {
                ...
                boolean currentlyVisible = (mStackScroller.getChildLocation(entry.row) & VISIBLE_LOCATIONS) != 0;
                if (currentlyVisible) {
            ...
            logNotificationVisibilityChanges(mTmpNewlyVisibleNotifications, noLongerVisibleNotifications);
    ...
    public void start() {
        ...
        int states = Settings.Global.getInt(mContext.getContentResolver(), "voice_wakeup_switch", 0);
        if(states == 1){
            ((ImageView)mNavigationBarView.getVoiceButton()).setImageResource(R.drawable.ic_sysbar_voice_on);
        }else{
            ((ImageView)mNavigationBarView.getVoiceButton()).setImageResource(R.drawable.ic_sysbar_voice_land);
        }
    }
    ...
    // ================================================================================
    protected AutoStatusBarView makeStatusBarView() {
        ...
        mStatusBarWindow.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (!isDeviceProvisioned()) {
                    if (DEBUG)
                        Log.e(TAG, "device not provisioned, ignore statusbar window touch event");
                    return true;
                }
        ...
        mStatusBarView = (AutoStatusBarView) mStatusBarWindow.findViewById(R.id.status_bar);
        mStatusBarView.setBar(this);

        PanelHolder holder = (PanelHolder) mStatusBarWindow.findViewById(R.id.panel_holder);
        mStatusBarView.setPanelHolder(holder);

        mAutoSettingPanel = (AutoSettingPanel) mStatusBarWindow.findViewById(R.id.notification_panel);
        mAutoSettingPanel.setStatusBar(this);
        mTalkingBarView = (TalkingStatusBarView) mStatusBarWindow.findViewById(R.id.auto_talking_status_bar);
        mTalkingBarView.registerTalkingbarStateListener(this);
        mNavigatorBarView = (NavigatorStatusBarView) mStatusBarWindow.findViewById(R.id.auto_navigator_status_bar);
        if(IS_WIDE_SCREEN){
            mStackFrameLayout = (NotificationStackFrameLayout)LayoutInflater.from(mContext).inflate(R.layout.notification_stack_layout, mStatusBarWindow, false);
            mStatusBarWindow.addView(mStackFrameLayout);
            mAutoNotificationManager = new AutoNotificationManager(context, mStackFrameLayout, this);
            mAutoNotificationManager.startListening(true);
        }
        ...
        if (!ActivityManager.isHighEndGfx() && !SystemProperties.getBoolean("ro.config.low_ram", true)) {
            ...
            mAutoSettingPanel.setBackground(new FastColorDrawable(
        ...
        if (MULTIUSER_DEBUG) {
            mNotificationPanelDebugText = (TextView) mAutoSettingPanel.findViewById(R.id.header_debug_info);
        ...
        SpeedBumpView speedBump = (SpeedBumpView) LayoutInflater.from(mContext)
                .inflate(R.layout.status_bar_notification_speed_bump, mStackScroller, false);
        ...
        mLocationController = new LocationControllerImpl(mContext); // will post
                                                                    // a
                                                                    // notification
        ...
        final SignalClusterView signalClusterKeyguard = (SignalClusterView) mKeyguardStatusBar
        ...
        mNetworkController.addEmergencyListener(new NetworkControllerImpl.EmergencyListener() {
            ...
            public void setEmergencyCallsOnly(boolean emergencyOnly) {
                ...
                if (DEBUG) {
                    Xlog.d(TAG, "setEmergencyCallsOnly: " + emergencyOnly + ", voiceFeature: " + isAPhone);
                }
                ...
                public void setCarrierLabel(String label) {
                    ...
                    if (mNetworkController.hasMobileDataFeature()) {
                        if (TextUtils.isEmpty(label)) {
                            ...
                            /// M: Support "Operator plugin - Customize Carrier
                            /// Label for PLMN" @{
        ...
        if (supportCustomizeCarrierLabel()) {
            mCustomizeCarrierLabel = mStatusBarPlmnPlugin.customizeCarrierLabel(mAutoSettingPanel, mCarrierLabel);
        }
        /// M: Support "Operator plugin - Customize Carrier Label for PLMN". @}
        ...
        if (mQSPanel != null) {
            final QSTileHost qsh = new QSTileHost(mContext, this, mBluetoothController, mLocationController,
                    mRotationLockController, mNetworkController, mZenModeController, mHotspotController,
                    mCastController, mFlashlightController, mUserSwitcherController, mKeyguardMonitor,
                    mSecurityController,
                    /// M: add HotKnot in quicksetting
                    mHotKnotController,
                    /// M: add AudioProfile in quicksetting
        ...
        TextView acc_stat = (TextView) mStatusBarView.findViewById(R.id.acc_stat);
        if (mCarcorderManager.queryCarEngineState() == 1)
            acc_stat.setVisibility(View.VISIBLE);
        else
            acc_stat.setVisibility(View.GONE);
        ...
        filter.addAction("aios.intent.action.STATE");
        filter.addAction("android.intent.action.HOMESTATE");
        filter.addAction(ACTION_DEMO);
        filter.addAction(Intent.ACTION_POWER_CONNECTED);
        filter.addAction(Intent.ACTION_POWER_DISCONNECTED);
        filter.addAction(NotificationConstant.RECORD_STATUS_ACTION);
        filter.addAction(NotificationConstant.PHONE_STATE_ACTION);
        filter.addAction(NotificationConstant.NAVIGATOR_HUD_CHANGED);
        filter.addAction(NotificationConstant.DRIVEMODE_STATUS_ACTION);
        filter.addAction(ACTION_ENABLE_TXZ_WAKEUP);
        filter.addAction(ACTION_DISABLE_TXZ_WAKEUP);
    ...
    public void onTalkingBarStateChange(boolean show){
        Log.d(TAG, "onTalkingBarStateChange: show " + show);
        mNavigatorBarView.updateBackground(show);
    }
        ...
        mStatusBarView.getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
            ...
            public boolean onPreDraw() {
                if (mDrawCount == 1) {
                    ...
                    HardwareCanvas.setProperty("extraRasterBucket", Float.toString(StackScrollAlgorithm.DIMMED_SCALE));
                    HardwareCanvas
                            .setProperty("extraRasterBucket",
                                    Float.toString(mContext.getResources().getDimensionPixelSize(
                                            R.dimen.qs_time_collapsed_size)
                                    / mContext.getResources().getDimensionPixelSize(R.dimen.qs_time_expanded_size)));
                }
                mDrawCount++;
                return true;
            }
    ...
    private void startKeyguard() {
        ...
        mStatusBarKeyguardViewManager = keyguardViewMediator.registerStatusBar(this, mStatusBarWindow,
                mStatusBarWindowManager, mScrimController);
    ...
    protected WindowManager.LayoutParams getSearchLayoutParams(LayoutParams layoutParams) {
        ...
        WindowManager.LayoutParams lp = new WindowManager.LayoutParams(LayoutParams.MATCH_PARENT,
                LayoutParams.MATCH_PARENT, WindowManager.LayoutParams.TYPE_NAVIGATION_BAR_PANEL,
    ...
    public void showSearchPanel() {
        if(factoryModeOn()){
            Log.d(TAG, "showSearchPanel: factory mode on, not show it! ");
            return;
        }
    ...
    public int getStatusBarHeight() {
        if (mNaturalBarHeight < 0) {
            ...
            mNaturalBarHeight = res.getDimensionPixelSize(com.android.internal.R.dimen.ali_status_bar_height);
        }
        // mNaturalBarHeight = 400;
        return mNaturalBarHeight;
    }

    public int getStatusBarWidth() {
        if (mNaturalBarWidth < 0) {
            final Resources res = mContext.getResources();
            mNaturalBarWidth = res.getDimensionPixelSize(com.android.internal.R.dimen.status_bar_width);
        }
        return mNaturalBarWidth;
    }

    public int getMiniStatusBarWidth() {
        if (mNaturalMiniBarWidth < 0) {
            final Resources res = mContext.getResources();
            mNaturalMiniBarWidth = res.getDimensionPixelSize(com.android.internal.R.dimen.ministatus_bar_width);
        }
        return mNaturalMiniBarWidth;
    }
    ...
    private View.OnClickListener mVoiceClickListener = new View.OnClickListener() {
        public void onClick(View v) {
            ...
            Intent voiceIntent = new Intent("com.aispeech.aios.UI_VOICE_TOGGLE");
            AliUserTrackUtil.ctrlClicked("Voice_Button", "Voice_Iconbtn");
            mContext.sendBroadcast(voiceIntent);
            // Log.v(TAG, "====zhujiong , send voice button broadcast");
        }
    };
    ...
    View.OnTouchListener mHomeActionListener = new View.OnTouchListener() {
        public boolean onTouch(View v, MotionEvent event) {
            ...
            /*
             * case MotionEvent.ACTION_DOWN: if (!shouldDisableNavbarGestures())
             * { mHandler.removeCallbacks(mShowSearchPanel);
    ...
    private void prepareNavigationBarView() {
        ...
        mNavigationBarView.getVoiceButton().setOnClickListener(mVoiceClickListener);
        if ("true".equals(SystemProperties.get("sys.voiceready", "")) && mNavigationBarView != null)
            mNavigationBarView.getVoiceButton().setVisibility(View.VISIBLE);
    ...
    public void addIcon(String slot, int index, int viewIndex, StatusBarIcon icon) {
        ...
        int inconheight = mContext.getResources().getDimensionPixelSize(R.dimen.ali_status_icon_size);
        StatusBarIconView view = new StatusBarIconView(mContext, slot, null);
        view.set(icon);
        LinearLayout.LayoutParams param = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, inconheight);
        param.bottomMargin = mContext.getResources().getDimensionPixelSize(R.dimen.status_bar_icon_margin);
        mStatusIcons.addView(view, viewIndex, param);
        view = new StatusBarIconView(mContext, slot, null);
        view.set(icon);
        mStatusIconsKeyguard.addView(view, viewIndex,
                new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, inconheight));
    }

    public void updateIcon(String slot, int index, int viewIndex, StatusBarIcon old, StatusBarIcon icon) {
        if (SPEW)
            Log.d(TAG, "updateIcon slot=" + slot + " index=" + index + " viewIndex=" + viewIndex + " old=" + old
                    + " icon=" + icon);
    ...
    public void addNotification(StatusBarNotification notification, RankingMap ranking) {
        // if (DEBUG) Log.d(TAG, "addNotification key=" +
        // notification.getKey());
        ...
        // if (DEBUG) Log.d(TAG, "Notification has fullScreenIntent; sending
        // fullScreenIntent");
        // try {
    ...
    public void removeNotification(String key, RankingMap ranking) {
        ...
        // if (SPEW) Log.d(TAG, "removeNotification key=" + key + " old=" +
        // old);
        //
        // if (old != null) {
    ...
    private void updateNotificationShade() {
        ...
        for (int i = 0; i < mStackFrameLayout.getChildCount(); i++) {
            View child = mStackFrameLayout.getChildAt(i);
        ...
        for (View remove : toRemove) {
            mStackFrameLayout.removeView(remove);
        }
        for (int i = 0; i < toShow.size(); i++) {
            View v = toShow.get(i);
            if (v.getParent() == null) {
                mStackFrameLayout.addView(v);
            }
        }

        // So after all this work notifications still aren't sorted correctly.
        // Let's do that now by advancing through toShow and mStackFrameLayout
        // in
        // lock-step, making sure mStackFrameLayout matches what we see in
        // toShow.
        int j = 0;
        for (int i = 0; i < mStackFrameLayout.getChildCount(); i++) {
            View child = mStackFrameLayout.getChildAt(i);
            ...
            mStackFrameLayout.changeViewPosition(toShow.get(j), i);
            j++;
        }
        updateRowStates();
        updateSpeedbump();
        ...
        // mAutoSettingPanel.setQsExpansionEnabled(isDeviceProvisioned()
    ...
    private boolean packageHasVisibilityOverride(String key) {
        return mNotificationData
                .getVisibilityOverride(key) != NotificationListenerService.Ranking.VISIBILITY_NO_OVERRIDE;
    }
    ...
    private void updateSpeedbump() {
        ...
        for (int i = 0; i < N; i++) {
            Entry entry = activeNotifications.get(i);
            if (entry.row.getVisibility() != View.GONE && mNotificationData.isAmbient(entry.key)) {
    ...
    private void updateNotificationIcons() {
        final LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(mIconSize + 2 * mIconHPadding,
                mNaturalBarHeight);
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (uniqueIcon.containsKey(key) && uniqueIcon.get(key) == ent.notification.getNotification().iconLevel) {
                // Log.d(TAG, "updateNotificationIcons(), IconMerger feature,
                // skip pkg / icon / iconlevel ="
        ...
        if (DEBUG) {
            Log.d(TAG,
                    "refreshing icons: " + toShow.size() + " notifications, mNotificationIcons=" + mNotificationIcons);
        }
    ...
    protected void updateCarrierLabelVisibility(boolean force) {
        ...
        if (SPEW) {
            Log.d(TAG, String.format("stackScrollerh=%d scrollh=%d carrierh=%d", mStackScroller.getHeight(),
                    mStackScroller.getHeight(), mCarrierLabelHeight));
        }

        // Emergency calls only is shown in the expanded header now.
        final boolean emergencyCallsShownElsewhere = true;
        final boolean makeVisible = !(emergencyCallsShownElsewhere && mNetworkController.isEmergencyOnly())
                && mStackScroller
                        .getHeight() < (mAutoSettingPanel.getHeight() - mCarrierLabelHeight - mStatusBarHeaderHeight)
                && mStackScroller.getVisibility() == View.VISIBLE && mState != StatusBarState.KEYGUARD;
        ...
        if (force || mCarrierLabelVisible != makeVisible) {
            ...
            mCarrierLabel.animate().alpha(makeVisible ? 1f : 0f)
                    // .setStartDelay(makeVisible ? 500 : 0)
                    // .setDuration(makeVisible ? 750 : 100)
                    .setDuration(150).setListener(makeVisible ? null : new AnimatorListenerAdapter() {
                        @Override
        ...
        synchronized (mNotificationData) {
            ...
            if (!sameSessions(mMediaController, controller)) {
                ...
                if (mMediaController != null) {
                    ...
                    final String notificationKey = mediaNotification == null ? null
                    ...
                    if (notificationKey == null || !notificationKey.equals(mMediaNotificationKey)) {
                        ...
                        if (DEBUG_MEDIA) {
                            Log.v(TAG, "DEBUG_MEDIA: Found new media notification: key=" + notificationKey
        ...
        if ((hasArtwork || DEBUG_MEDIA_FAKE_ARTWORK)
            ...
            if (metaDataChanged) {
                ...
                if (DEBUG_MEDIA_FAKE_ARTWORK) {
                    ...
                    // Log.v(TAG, String.format("DEBUG_MEDIA: setting new color:
                    // 0x%08x", c));
            ...
            if (mBackdrop.getVisibility() != View.GONE) {
                ...
                mBackdrop.animate().alpha(0f).setInterpolator(mBackdropInterpolator).setDuration(300).setStartDelay(0)
                ...
                if (mKeyguardFadingAway) {
                    ...
                    // Make it disappear faster, as the focus should be on the
                    // activity behind.
                            .setDuration(mKeyguardFadingAwayDuration / 2).setStartDelay(mKeyguardFadingAwayDelay)
                            .setInterpolator(mLinearInterpolator).start();
                }
            }
        }
    }
    ...
    private int adjustDisableFlags(int state) {
        if (!mLaunchTransitionFadingAway && (mExpandedVisible || mBouncerShowing || mWaitingForKeyguardExit)) {
    ...
    public void disable(int state, boolean animate) {
        ...
        if ((diff & (StatusBarManager.DISABLE_HOME | StatusBarManager.DISABLE_RECENT | StatusBarManager.DISABLE_BACK
    ...
    private void animateStatusBarHide(final View v, boolean animate) {
        ...
        v.animate().alpha(0f).setDuration(160).setStartDelay(0).setInterpolator(ALPHA_OUT)
                .withEndAction(new Runnable() {
                    @Override
                    public void run() {
                        v.setVisibility(View.INVISIBLE);
    ...
    private void animateStatusBarShow(View v, boolean animate) {
        ...
        v.animate().alpha(1f).setDuration(320).setInterpolator(ALPHA_IN).setStartDelay(50)

        // We need to clean up any pending end action from animateStatusBarHide
        // if we call
        // both hide and show in the same frame before the animation actually
        // gets started.
        ...
        if (mKeyguardFadingAway) {
            v.animate().setDuration(mKeyguardFadingAwayDuration).setInterpolator(mLinearOutSlowIn)
                    .setStartDelay(mKeyguardFadingAwayDelay).start();
        }
    }
    ...
    public boolean isQsExpanded() {
        // TODO:fix compile error
        return false;
        // return mAutoSettingPanel.isQsExpanded();
    }
    ...
    public Animator setVisibilityWhenDone(final Animator a, final View v, final int vis) {
    ...
    public void animateExpandSettingsPanel() {
        ...
        if (!mUserSetup)
            return;
        // TODO:fix compile error
        // mAutoSettingPanel.expandWithQs();
    ...
    public void makeExpandedInvisible() {
        ...
        mAutoSettingPanel.setVisibility(View.GONE);
        // TODO:fix compile error
        // mAutoSettingPanel.closeQs();
    ...
    public boolean interceptTouchEvent(MotionEvent event) {
        if (DEBUG_GESTURES) {
            if (event.getActionMasked() != MotionEvent.ACTION_MOVE) {
                EventLog.writeEvent(EventLogTags.SYSUI_STATUSBAR_TOUCH, event.getActionMasked(), (int) event.getX(),
        ...
        if (SPEW) {
            Log.d(TAG, "Touch: rawY=" + event.getRawY() + " event=" + event + " mDisabled=" + mDisabled + " mTracking="
                    + mTracking);
        } else if (CHATTY) {
            if (event.getAction() != MotionEvent.ACTION_MOVE) {
                Log.d(TAG, String.format("panel: %s at (%f, %f) mDisabled=0x%08x",
                        MotionEvent.actionToString(event.getAction()), event.getRawX(), event.getRawY(), mDisabled));
            }
        }
    ...
    protected void updateBarExpandStatus() {
        // setup status bar is collapse,
        // when setup completed, we need expand status.
        if (IS_WIDE_SCREEN && mStatusBarWindowManager != null) {// status bar is ready.
            expandStatusBar();
        }
    }

    private void expandStatusBar() {
        Animation anim = AnimationUtils.loadAnimation(mContext, com.android.internal.R.anim.slide_in_left);
        mStackFrameLayout.setVisibility(View.VISIBLE);
        mStackFrameLayout.startAnimation(anim);
        // WindowManager.LayoutParams lp =
        // (android.view.WindowManager.LayoutParams)mStatusBarWindow.getLayoutParams();
        // lp.width = getStatusBarWidth();
        // mWindowManager.updateViewLayout(mStatusBarWindow, lp);
        mStatusBarWindowManager.setStackLayoutExpanded(true);
    }

    private void collapseStatusBar() {
        Animation anim = AnimationUtils.loadAnimation(mContext, com.android.internal.R.anim.slide_out_left);
        anim.setAnimationListener(new Animation.AnimationListener() {
            ...
            public void onAnimationEnd(Animation animation) {
                mStackFrameLayout.setVisibility(View.GONE);
                mStatusBarWindowManager.setStackLayoutExpanded(false);
            }
        });
        mStackFrameLayout.startAnimation(anim);
        // mStackFrameLayout.setVisibility(View.VISIBLE);
        // WindowManager.LayoutParams lp =
        // (android.view.WindowManager.LayoutParams)mStatusBarWindow.getLayoutParams();
        // lp.width = getMiniStatusBarWidth();
        // mWindowManager.updateViewLayout(mStatusBarWindow, lp);
    }
    ...
    public void setWindowState(int window, int state) {
        ...
        if (mStatusBarWindow != null && window == StatusBarManager.WINDOW_STATUS_BAR
            ...
            if (DEBUG_WINDOW_STATE)
                Log.d(TAG, "setWindowState showing = " + showing);
            if(IS_WIDE_SCREEN){
                if (showing) {
                    expandStatusBar();
                } else {
    ...
    public void setInteracting(int barWindow, boolean interacting) {
        ...
        // TODO: fix ME, compile error
        if ((RecentsActivity.FLOAT_WINDOW_SUPPORT)/* &&isFloatPanelOpened() */) {/// M:
                                                                                    /// [SystemUI]
                                                                                    /// [Tablet
                                                                                    /// Only]Support
                                                                                    /// "Multi
    ...
    protected void tick(StatusBarNotification n, boolean firstTime) {
        ...
        // if (n.getNotification().tickerText != null && mStatusBarWindow !=
        // null
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        pw.println("  Panels: ");
        if (mAutoSettingPanel != null) {
            pw.println("    mNotificationPanel=" + mAutoSettingPanel + " params="
                    + mAutoSettingPanel.getLayoutParams().debug(""));
            pw.print("      ");
            mAutoSettingPanel.dump(fd, pw, args);
        }
        ...
        for (Map.Entry<String, ?> entry : mContext.getSharedPreferences(mContext.getPackageName(), Context.MODE_PRIVATE)
                .getAll().entrySet()) {
            pw.print("  ");
    ...
    private void addStatusBarWindow() {
        ...
        mStatusBarWindowManager.add(mStatusBarWindow, getStatusBarWidth(), getMiniStatusBarWidth(),
    ...
    public void updateExpandedViewPos(int thingy) {
        ...
        mAutoSettingPanel.setMinimumHeight((int) (mNotificationPanelMinHeightFrac * mCurrentDisplaySize.y));

        FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) mAutoSettingPanel.getLayoutParams();
        // lp.gravity = mNotificationPanelGravity;
        lp.gravity = Gravity.LEFT | Gravity.FILL_VERTICAL;
        mAutoSettingPanel.setLayoutParams(lp);
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            isTxzEnable = intent.getBooleanExtra("txz", false);
            Log.d(TAG, "[PhoneStatusBar] isTxzEnable:"+isTxzEnable);
            if (Intent.ACTION_CLOSE_SYSTEM_DIALOGS.equals(action)) {
            ...
            } else if ("aios.intent.action.STATE".equals(action)) {
                String extra = intent.getStringExtra("aios.intent.extra.STATE");
                if ("ready".equals(extra)) {
                    mNavigationBarView.getVoiceButton().setVisibility(View.VISIBLE);
                    SystemProperties.set("sys.voiceready", "true");
                }else{
                    mNavigationBarView.getVoiceButton().setVisibility(View.INVISIBLE);
                    SystemProperties.set("sys.voiceready", "false");
                }
                if (DEBUG)
                    Log.d(TAG, "extra = " + extra);
                animateCollapsePanels();
            } else if ("android.intent.action.HOMESTATE".equals(action)) {
                String extra = intent.getStringExtra("HOMESTATE");
                // Log.w(TAG, "==========zhujiong, HOMESTATE = " + extra);
                KeyButtonView homeButton = (KeyButtonView) (mNavigationBarView.getHomeButton());
                if ("home".equals(extra)) {
                    homeButton.setImageResource(R.drawable.ic_sysbar_home_land);
                } else {
                    homeButton.setImageResource(R.drawable.ic_sysbar_home_subpage);
                }
                ...
                if (mCarcorderManager.queryCarEngineState() == 1)
                    ...
                    acc_stat.setVisibility(View.GONE);
            } else if (NotificationConstant.RECORD_STATUS_ACTION.equals(action)) {
                int status = intent.getIntExtra(NotificationConstant.KEY_RECORD_STATUS,
                        NotificationConstant.RECORD_STATUS_STOP);
                ImageView recordstatus = (ImageView) mStatusBarView.findViewById(R.id.dvr_stat);
                recordstatus.setImageResource(status == NotificationConstant.RECORD_STATUS_START ? R.drawable.recording
                        : R.drawable.recording_stop);
            } else if (NotificationConstant.PHONE_STATE_ACTION.equals(action)) {
                int state = intent.getIntExtra(NotificationConstant.KEY_PHONE_STATE,
                        NotificationConstant.PHONE_STATE_IDLE);
                int show = intent.getIntExtra(NotificationConstant.KEY_SHOW_TALKING_BAR,
                        NotificationConstant.HIDE_TALKING_BAR);
                long startTime = intent.getLongExtra(NotificationConstant.KEY_PHONE_START_TIME, -1);
                Log.d(TAG, "receive " + NotificationConstant.PHONE_STATE_ACTION + ", state = " + state
                        + ", talking bar show = " + show + ", startTime = " + startTime);
                mTalkingBarView.setStatus(state, show, startTime);
            } else if (NotificationConstant.NAVIGATOR_HUD_CHANGED.equals(action)){
                // update navigator status bar.
                int state = intent.getIntExtra(NotificationConstant.KEY_NAVIGATOR_HUD_STATE,
                        -1);
                int icon = intent.getIntExtra(NotificationConstant.KEY_NAVIGATOR_HUD_ICON,
                        -1);
                int dis = intent.getIntExtra(NotificationConstant.KEY_NAVIGATOR_SEG_REMAIN_DIS,
                        -1);
                mNavigatorBarView.updateNavigatorStatus(state, icon, dis);
            } else if (NotificationConstant.DRIVEMODE_STATUS_ACTION.equals(action)){
                int on = intent.getIntExtra(NotificationConstant.KEY_DRIVEMODE_STATUS,
                        NotificationConstant.DRIVEMODE_OFF);
                mNavigatorBarView.switchDriveMode(on == NotificationConstant.DRIVEMODE_ON);
            } else if (ACTION_ENABLE_TXZ_WAKEUP.equals(action)) {
                ((ImageView)mNavigationBarView.getVoiceButton()).setImageResource(R.drawable.ic_sysbar_voice_on);
            } else if (ACTION_DISABLE_TXZ_WAKEUP.equals(action)) {
                ((ImageView)mNavigationBarView.getVoiceButton()).setImageResource(R.drawable.ic_sysbar_voice_land);
            }
        }
    };
    ...
    private void setHeadsUpVisibility(boolean vis) {
        ...
        EventLog.writeEvent(EventLogTags.SYSUI_HEADS_UP_STATUS, vis ? mHeadsUpNotificationView.getKey() : "",
                vis ? 1 : 0);
    ...
    void updateResources() {
        ...
        // TODO:fix compile error
        // if (mAutoSettingPanel != null) {
        // mAutoSettingPanel.updateResources();
        // }
    ...
    protected void loadDimens() {
        ...
        if (newIconHPadding != mIconHPadding) {
    ...
    private void logStateToEventlog() {
        ...
        if (stateFingerprint != mLastLoggedStateFingerprint) {
            EventLogTags.writeSysuiStatusBarState(mState, isShowing ? 1 : 0, isOccluded ? 1 : 0,
                    isBouncerShowing ? 1 : 0, isSecure ? 1 : 0, isCurrentlyInsecure ? 1 : 0);
    ...
    protected boolean shouldDisableNavbarGestures() {
        return !isDeviceProvisioned() || mExpandedVisible || (mDisabled & StatusBarManager.DISABLE_SEARCH) != 0;
    }
    ...
    public void showKeyguard() {
        if (mLaunchTransitionFadingAway) {
            mAutoSettingPanel.animate().cancel();
            mAutoSettingPanel.setAlpha(1f);
        ...
        // with a blank
        // // Keyguard.
        // mAutoSettingPanel.setTouchDisabled(true);
        // }
    ...
    public boolean isCollapsing() {
        return mAutoSettingPanel.isCollapsing();
    }
    ...
    public boolean isInLaunchTransition() {
        return false;
        // TODO:fix compile error
        // return mAutoSettingPanel.isLaunchTransitionRunning()
        // || mAutoSettingPanel.isLaunchTransitionFinished();
    }
    ...
    public void fadeKeyguardAfterLaunchTransition(final Runnable beforeFading, Runnable endRunnable) {
        ...
        Runnable hideRunnable = new Runnable() {
            ...
            public void run() {
                ...
                mAutoSettingPanel.setAlpha(1);
                mAutoSettingPanel.animate().alpha(0).setStartDelay(FADE_KEYGUARD_START_DELAY)
        ...
        // TODO:fix compile error
        // if (mAutoSettingPanel.isLaunchTransitionRunning()) {
        // mAutoSettingPanel.setLaunchTransitionEndRunnable(hideRunnable);
        // } else {
        // hideRunnable.run();
        // }
    }
    ...
    public boolean hideKeyguard() {
        ...
        if (mLeaveOpenOnKeyguardHide) {
            mLeaveOpenOnKeyguardHide = false;
            // TODO:fix compile error
            // mAutoSettingPanel.animateToFullShade(calculateGoingToFullShadeDelay());
            if (mDraggedDownRow != null) {
                mDraggedDownRow.setUserLocked(false);
                mDraggedDownRow = null;
            }
    ...
    private void updatePublicMode() {
        setLockscreenPublicMode(
                mStatusBarKeyguardViewManager.isShowing() && mStatusBarKeyguardViewManager.isSecure(mCurrentUserId));
    }
    ...
    private void updateKeyguardState(boolean goingToFullShade, boolean fromShadeLocked) {
        if (mState == StatusBarState.KEYGUARD) {
            ...
            // goingToFullShade || mState == StatusBarState.SHADE_LOCKED ||
            // fromShadeLocked);
        }
        if (mState == StatusBarState.KEYGUARD || mState == StatusBarState.SHADE_LOCKED) {
            // mScrimController.setKeyguardShowing(true);
            android.util.Log.d("Huxs", "updateKeyguardState mState = " + mState + ", goingToFullShade = "
                    + goingToFullShade + ", fromShadeLocked = " + fromShadeLocked);
            mScrimController.setKeyguardShowing(false);
        } else {
            mScrimController.setKeyguardShowing(false);
            android.util.Log.d("Huxs", "updateKeyguardState mState = " + mState + ", XXXXXXXXX");
        }
    ...
    public boolean onBackPressed() {
        ...
        // TODO:fix compile error
        // if (mAutoSettingPanel.isQsExpanded()) {
        // if (mAutoSettingPanel.isQsDetailShowing()) {
        // mAutoSettingPanel.closeQsDetail();
        // } else {
        // mAutoSettingPanel.animateCloseQs();
        // }
        // return true;
        // }
        if (mState != StatusBarState.KEYGUARD && mState != StatusBarState.SHADE_LOCKED) {
    ...
    private void instantExpandNotificationsPanel() {
        ...
        // TODO:fix compile error
        // mAutoSettingPanel.instantExpand();
    }

    private void instantCollapseNotificationPanel() {
        // TODO:fix compile error
        // mAutoSettingPanel.instantCollapse();
    }

    @Override
    public void onActivated(ActivatableNotificationView view) {
        EventLogTags.writeSysuiLockscreenGesture(EventLogConstants.SYSUI_LOCKSCREEN_GESTURE_TAP_NOTIFICATION_ACTIVATE,
    ...
    public boolean onDraggedDown(View startingChild, int dragLengthY) {
        if (hasActiveNotifications()) {
            EventLogTags.writeSysuiLockscreenGesture(EventLogConstants.SYSUI_LOCKSCREEN_GESTURE_SWIPE_DOWN_FULL_SHADE,
    ...
    public void setEmptyDragAmount(float amount) {
        // TODO:fix compile error
        // mAutoSettingPanel.setEmptyDragAmount(amount);
    }
    ...
    public void goToLockedShade(View expandView) {
        ...
        if (isLockscreenPublicMode() && fullShadeNeedsBouncer) {
            ...
            // TODO:fix compile error
            // mAutoSettingPanel.animateToFullShade(0 /* delay */);
    ...
    /**
     * @return a ViewGroup that spans the entire panel which contains the quick
     *         settings
     */
    public ViewGroup getQuickSettingsOverlayParent() {
        return mAutoSettingPanel;
    }
    ...
    public void onScreenTurnedOn() {
        ...
        // TODO:fix compile error
        // mAutoSettingPanel.setTouchDisabled(false);
        updateVisibleToUser();
    }

    /**
     * This handles long-press of both back and recents. They are handled
     * together to capture them both being long-pressed at the same time to exit
     * screen pinning (lock task).
     *
     * When accessibility mode is on, only a long-press from recents is required
     * to exit.
     *
     * In all other circumstances we try to pass through long-press events for
     * Back, so that apps can still use it. Which can be from two things. 1) Not
     * currently in screen pinning (lock task). 2) Back is long-pressed without
     * recents.
     */
        ...
        try {
            ...
            if (activityManager.isInLockTaskMode() && !isAccessiblityEnabled) {
                ...
                /* && !mNavigationBarView.getRecentsButton().isPressed() */) {
                    ...
                    // won't be together, so send the standard long-press
                    // action.
                    sendBackLongPress = true;
                }
                mLastLockToAppLongPress = time;
            } else {
                // If this is back still need to handle sending the long-press
                // event.
                if (v.getId() == R.id.back) {
                    sendBackLongPress = true;
                } else if (isAccessiblityEnabled && activityManager.isInLockTaskMode()) {
                    // When in accessibility mode a long press that is recents
                    // (not back)
     ...
     *
     * @param businessType
     *            public static final String VOICE_CALL_SIM_SETTING =
     *            "voice_call_sim_setting"; public static final String
     *            SMS_SIM_SETTING = "sms_sim_setting"; Default value public
     *            static final long VOICE_CALL_SIM_SETTING_INTERNET = -2; public
     *            static final long DEFAULT_SIM_SETTING_ALWAYS_ASK = -1; public
     *            static final long SMS_SIM_SETTING_AUTO = -3;
     */
    public void showSimIndicator(String businessType) {
        ...
        case Settings.System.VOICE_CALL_SIM_SETTING:
            ...
            if ((int) Settings.System.DEFAULT_SIM_SETTING_ALWAYS_ASK == mSubIDinSetting
                ...
                // Xlog.d(TAG, "showSimIndicator Voice Call Setting= " +
                // mSubIDinSetting);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_SMARTBOOK_PLUG.equals(action)) {
                ...
                if (mPlugin != mIsDisplayDevice) {
                    // Log.v(TAG, "recreateStatusBar, mIsDisplayDevice = " +
                    // mIsDisplayDevice);
    ...
    public void processNotificationKeyEvent(KeyEvent event) {
        ...
        if (down && repeatCount == 0) {
            ...
            if (mAutoSettingPanel.isFullyExpanded()) {
                isNotificationPanelShow = true;
            }
            // Log.v(TAG, "processOnKeyEvent, KEYCODE_NOTIFICATION, isNPanel = "
            // + isNotificationPanelShow
            // + " mExpandedVisible= " + mExpandedVisible + " mQuicksettingShow=
            // " + mQuicksettingShow);

            if ((metaState & KeyEvent.META_SHIFT_RIGHT_ON) != 0 || (metaState & KeyEvent.META_SHIFT_LEFT_ON) != 0) {
    ...
    public void ChangeTalkingStatusBar(String pkg, int id, int type, Notification notification) {
    }
    ...
    public void allowTalkingBarShow(boolean allow) {
    }

    public void animateCollapse() {
    }

    public void updateSystemUI(int state) {
    }

    public void updateRefreshButton(boolean isShow, int width) {
    }

    public void disableNavBarButton(int state) {
    }
    ...
    private final void updateCustomizeCarrierLabelVisibility(boolean force) {
        if (DEBUG) {
            Log.d(TAG, "updateCustomizeCarrierLabelVisibility(), force = " + force + ", mState = " + mState);
        }
	...
	public boolean getStatusBarEnable() {
		try {
			int value = Settings.Global.getInt(mContext.getContentResolver(),Settings.Global.STATUSBAR_ENABLE,1);
			Log.d(TAG,"StatusBarEnable value="+value);
			if (value != 1) {
				return false;
			}
		} catch (Exception e) {
