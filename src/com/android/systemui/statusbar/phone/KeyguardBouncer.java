...
import com.android.keyguard.AliKeyguardHostView;
...
public class KeyguardBouncer {
    ...
    /*YUNOS BEGIN*/
  //##modules(Keyguard): [use AliKeyguardHostView]
  //##date:2014/04/16 ##kerong.skr
      private AliKeyguardHostView mKeyguardView;

      public void goToUnlockScreen(boolean unlock, PendingIntent pending) {
          if (mKeyguardView != null)
              mKeyguardView.goToUnlockScreen(unlock, pending);
      }

      public void goToLockScreen() {
          if (mKeyguardView != null)
              mKeyguardView.goToLockScreen();
      }
  /*YUNOS END*/
...
//            mKeyguardView.showPrimarySecurityScreen();
        ...
        //if (!mKeyguardView.dismiss(authenticated)) {
        	//Log.d(TAG, "mKeyguardView.dismiss()");
       //     if (DEBUG) {
...
//            mKeyguardView.showPrimarySecurityScreen();
    ...
    private void inflateView() {
        ...
        mKeyguardView = new AliKeyguardHostView(mContext, mLockPatternUtils);
        mKeyguardView.setViewMediatorCallback(mCallback);
        mKeyguardView.setId(R.id.keyguard_host_view);
        mRoot.addView(mKeyguardView);
        ...
        mWindowLayoutParams = mWindowManager.getWindowLayoutParams();
        /*YUNOS BEGIN*/
      //##modules(Keyguard): [use AliKeyguardHostView update WindowLayoutParams]
      //##date:2014/04/16 ##kerong.skr
        AliKeyguardHostView.updateWindowLayoutParams(mWindowLayoutParams);
      /*YUNOS END*/
        if (mScreenOn) {
        	mKeyguardView.onScreenTurnedOn();
        }
        ...
        //##modules(Keyguard): [use AliKeyguardHostView update WindowLayoutParams]
        //##date:2014/04/16 ##kerong.skr
                mKeyguardView.setWindowLayoutParams(mWindowLayoutParams);
                mKeyguardView.setWindowLayoutUpdateRunnable(new Runnable() {
                    public void run() {
                    	mWindowManager.getWindowManager().updateViewLayout(mContainer, mWindowLayoutParams);
                    }
                });
        /*YUNOS END*/
        //mRoot.setVisibility(View.INVISIBLE);
    ...
    public boolean isFullscreenBouncer() {
        if (mKeyguardView != null) {
		//TODO: fix ME, compile error
            SecurityMode mode = null/*mKeyguardView.getCurrentSecurityMode()*/;
            return mode == SecurityMode.SimPinPukMe1
                || mode == SecurityMode.SimPinPukMe2
                || mode == SecurityMode.SimPinPukMe3
                || mode == SecurityMode.SimPinPukMe4
    ...
    public boolean isSecure() {
        return mKeyguardView == null || mKeyguardView.getSecurityMode() != AliKeyguardHostView.SecurityMode.None;
    }
