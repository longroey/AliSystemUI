...
public class PanelView extends FrameLayout {
    ...
    private float mExpandedWidth = 0;
    ...
    private void runPeekAnimation() {
        if (DEBUG) logf("peek to height=%.1f", mPeekWidth);
        if (mTimeAnimator.isStarted()) {
            return;
        }
        if (mPeekAnimator == null) {
            mPeekAnimator = ObjectAnimator.ofFloat(this,
                    "expandedWidth", mPeekWidth)
                .setDuration(250);
        }
        mPeekAnimator.start();
    }
    ...
    private void animationTick(long dtms) {
        if (!mTimeAnimator.isStarted()) {
            ...
            mRubberbanding = mRubberbandingEnabled // is it enabled at all?
                    && mExpandedWidth > getFullWidth() // are we past the end?
                    && mVel >= -mFlingGestureMinDistPx; // was this not possibly a "close" gesture?
            if (mRubberbanding) {
                mClosing = true;
            } else if (mVel == 0) {
                // if the panel is less than halfway open, close it
                mClosing = (mFinalTouchX / getFullWidth()) < 0.5f;
            } else {
                mClosing = mExpandedWidth > 0 && mVel < 0;
            }
        } else if (dtms > 0) {
            final float dt = dtms * 0.001f;                  // ms -> s
            if (DEBUG) logf("tick: v=%.2fpx/s dt=%.4fs", mVel, dt);
            if (DEBUG) logf("tick: before: h=%d", (int) mExpandedWidth);

            final float fw = getFullWidth();
            boolean braking = false;
            if (BRAKES) {
                if (mClosing) {
                    braking = mExpandedWidth <= mCollapseBrakingDistancePx;
                    mAccel = braking ? 10*mCollapseAccelPx : -mCollapseAccelPx;
                } else {
                    braking = mExpandedWidth >= (fw-mExpandBrakingDistancePx);
            ...
            float w = mExpandedWidth + mVel * dt;

            if (mRubberbanding && w < fw) {
                w = fw;
            }

            if (DEBUG) logf("tick: new h=%d closing=%s", (int) w, mClosing?"true":"false");

            setExpandedWidthInternal(w);

            mBar.panelExpansionChanged(PanelView.this, mExpandedFraction);

            if (mVel == 0
                    || (mClosing && mExpandedWidth == 0)
                    || ((mRubberbanding || !mClosing) && mExpandedWidth == fw)) {
                post(mStopAnimator);
            }
        } else {
            Log.v(TAG, "animationTick called with dtms=" + dtms + "; nothing to do (w="
                    + mExpandedWidth + " v=" + mVel + ")");
        }
    }
    ...
    private void loadDimens() {
        ...
        mPeekWidth = res.getDimension(R.dimen.peek_height)
            + getPaddingLeft() // our window might have a dropshadow
            - (mHandleView == null ? 0 : mHandleView.getPaddingLeft()); // the handle might have a topshadow
    }
        ...
        if (mHandleView != null) {
            mHandleView.setOnTouchListener(new View.OnTouchListener() {
                ...
                public boolean onTouch(View v, MotionEvent event) {
                    ...
                    final float rawDelta = event.getRawX() - event.getX();
                    final float rawX = x + rawDelta;
                    ...
                    switch (event.getActionMasked()) {
                        case MotionEvent.ACTION_DOWN:
                            ...
                            mTouchOffset = (rawX - mAbsPos[0]) - mExpandedWidth;
                            if (mExpandedWidth == 0) {
                                mJustPeeked = true;
                                runPeekAnimation();
                            }
                        ...
                        case MotionEvent.ACTION_POINTER_UP:
                            ...
                            if (mTrackingPointer == upPointer) {
                                ...
                                final float newRawX = newX + rawDelta;
                                mTrackingPointer = event.getPointerId(newIndex);
                                mTouchOffset = (newRawX - mAbsPos[0]) - mExpandedWidth;
                                mInitialTouchX = newX;
                            }
                            break;

                        case MotionEvent.ACTION_MOVE:
                            final float w= rawX - mAbsPos[0] - mTouchOffset;
                            if (w > mPeekWidth) {
                            ...
                            if (!mJustPeeked) {
                                PanelView.this.setExpandedWidthInternal(w);
                                mBar.panelExpansionChanged(PanelView.this, mExpandedFraction);
                            }
                        ...
                        case MotionEvent.ACTION_CANCEL:
                            ...
                            if (mVelocityTracker != null) {
                                ...
                                if (yVel > mFlingGestureMaxXVelocityPx) {
                                    yVel = mFlingGestureMaxXVelocityPx; // limit how much we care about the x axis
                                }
                            ...
                            final float deltaX = Math.abs(mFinalTouchX - mInitialTouchX);
                            if (deltaX < mFlingGestureMinDistPx
...
//        setMeasuredDimension(widthMeasureSpec, heightMeasureSpec);
        ...
        if(newWidth != mFullWidth){
        	mFullWidth = newWidth;
        	if(!mTracking && !mRubberbanding && !mTimeAnimator.isStarted()
        			&& mExpandedWidth > 0 && mExpandedWidth != mFullWidth){
        		mExpandedWidth = mFullWidth;
        	}
        }
        widthMeasureSpec = MeasureSpec.makeMeasureSpec(
                (int) mExpandedWidth, MeasureSpec.AT_MOST);
        setMeasuredDimension(widthMeasureSpec, heightMeasureSpec);
    }
...
//    }
    ...
    public void setExpandedWidth(float width) {
        if (DEBUG) logf("setExpandedWidth(%.1f)", width);
        mRubberbanding = false;
        if (mTimeAnimator.isStarted()) {
            post(mStopAnimator);
        }
        setExpandedWidthInternal(width);
        mBar.panelExpansionChanged(PanelView.this, mExpandedFraction);
    }
...
//        if (DEBUG) logf("onLayout: changed=%s, bottom=%d eh=%d fh=%d", changed?"T":"f", bottom, (int)mExpandedHeight, mFullHeight);
        if (DEBUG) logf("onLayout: changed=%s, left=%d top=%d right=%d eh=%d fh=%d", changed?"T":"f", left, top, right, (int)mExpandedWidth, mFullWidth);
...
//    }
    ...
    public void setExpandedWidthInternal(float w) {
        if (Float.isNaN(w)) {
            // If a NaN gets in here, it will freeze the Animators.
            if (DEBUG_NAN) {
                Log.v(TAG, "setExpandedWidthInternal: warning: w=NaN, using 0 instead",
                        new Throwable());
            }
            w = 0;
        }

        float fw = getFullWidth();
        if (fw == 0) {
            // Hmm, full height hasn't been computed yet
        }

        if (w < 0) w = 0;
        if (!(mRubberbandingEnabled && (mTracking || mRubberbanding)) && w > fw) w = fw;

        mExpandedWidth = w;

        if (DEBUG) logf("setExpansion: height=%.1f fh=%.1f tracking=%s rubber=%s", w, fw, mTracking?"T":"f", mRubberbanding?"T":"f");
...
//        setLayoutParams(lp);
        ...
        mExpandedFraction = Math.min(1f, (fw == 0) ? 0 : w / fw);
    }
...
//    }
    ...
    private float getFullWidth() {
        if (mFullWidth <= 0) {
            if (DEBUG) logf("Forcing measure() since mFullWidth=" + mFullWidth);
            measure(MeasureSpec.makeMeasureSpec(android.view.ViewGroup.LayoutParams.WRAP_CONTENT, MeasureSpec.EXACTLY),
                    MeasureSpec.makeMeasureSpec(android.view.ViewGroup.LayoutParams.WRAP_CONTENT, MeasureSpec.EXACTLY));
        }
        return mFullWidth;
    ...
    public void setExpandedFraction(float frac) {
        ...
        setExpandedWidth(getFullWidth() * frac);
    }
    ...
    public boolean isFullyExpanded() {
        return mExpandedWidth >= getFullWidth();
    }

    public boolean isFullyCollapsed() {
        return mExpandedWidth <= 0;
    }
