...
import com.android.keyguard.AliKeyguardUtils;
...
public class KeyguardViewMediator extends SystemUI {
    ...
    private static final boolean DEBUG = false; //KeyguardConstants.DEBUG;
    private static final boolean DEBUG_SIM_STATES = false; //KeyguardConstants.DEBUG_SIM_STATES;
    private final static boolean DBG_WAKE = false;
    private final static boolean DBG_MESSAGE = false;
...
//                            Log.d(TAG, "Get NETWORK_LOCKED but not support ME lock. Not show.");
                        ...
                        /*YUNOS BEGIN*/
                      //##modules(Keyguard): [clean up PIN PUK dismiss flag]
                      //##date:2014/04/04 ##kerong.skr
                                          /*if ((IccCardConstants.State.PIN_REQUIRED == simState && AliKeyguardUtils
                                                  .getPINDismiss(simId, AliKeyguardUtils.SIM_LOCK_PIN))
                                                  || (IccCardConstants.State.PUK_REQUIRED == simState && AliKeyguardUtils
                                                          .getPINDismiss(simId, AliKeyguardUtils.SIM_LOCK_PUK))) {
                                              AliKeyguardUtils.setPINDismiss(simId, AliKeyguardUtils.SIM_LOCK_PIN, false);
                                              AliKeyguardUtils.setPINDismiss(simId, AliKeyguardUtils.SIM_LOCK_PUK, false);
                                          }*/
                      /*YUNOS END*/
...
//                                        "And do not show lock screen.") ;
                        ...
                        if (!isShowing()) {
                            ...
                            /*YUNOS BEGIN*/
                          //##modules(Keyguard): [directly goto SIM unlock screen]
                          //##date:2014/10/29 ##kerong.skr
                                                  /*mHandler.post(new Runnable() {
                                                      public void run() {
                                                          mStatusBarKeyguardViewManager.goToUnlockScreen(false, null);
                                                      }
                                                  });*/
                          /*YUNOS END*/
    ...
    private void setupLocked() {
        ...
        /*YUNOS BEGIN*/
      //##modules(Keyguard): [add broadcast receiver to toggle Keyguard]
      //##date:2014/02/21 ##kerong.skr
              IntentFilter lockFilter = new IntentFilter(ALIYUN_KEYGUARD_LOCK_ACTION);
              lockFilter.addAction(ALIYUN_KEYGUARD_UNLOCK_ACTION);
              lockFilter.addAction(ALIYUN_KEYGUARD_GOTO_LOCKSCREEN_ACTION);
              lockFilter.addAction(ALIYUN_KEYGUARD_GOTO_UNLOCKSCREEN_ACTION);
              mContext.registerReceiver(mToggleBroadcastReceiver, lockFilter);
              AliKeyguardUtils.init(mContext);
      /*YUNOS END*/
    ...
    public void onScreenTurnedOff(int why) {
        ...
        synchronized (this) {
            ...
            if (mExitSecureCallback != null) {
                ...
                /*YUNOS BEGIN*/
                //##modules(Keyguard): [notify screen off]
                //##date:2014/01/19 author:kerong.skr
                notifyScreenOffLocked();
                /*YUNOS END*/
                doKeyguardLocked(null);
            }
...
//            ", keyguardisable=" + keyguardDisable + ", antiTheftLocked=" + antiTheftLocked);
  ...
  //##date:2014/02/21 ##kerong.skr
      ...
      private static final String ALIYUN_KEYGUARD_UNLOCK_ACTION = "aliyun.intent.action.KEYGUARD_UNLOCK";
      private static final String ALIYUN_KEYGUARD_GOTO_LOCKSCREEN_ACTION = "aliyun.intent.action.KEYGUARD_GOTO_LOCKSCREEN";
      private static final String ALIYUN_KEYGUARD_GOTO_UNLOCKSCREEN_ACTION = "aliyun.intent.action.KEYGUARD_GOTO_UNLOCKSCREEN";

      private final BroadcastReceiver mToggleBroadcastReceiver = new BroadcastReceiver() {
          @Override
          public void onReceive(Context context, Intent intent) {
              String action = intent.getAction();
              if (ALIYUN_KEYGUARD_LOCK_ACTION.equals(action)) {
                  showLocked(null);
              } else if (ALIYUN_KEYGUARD_UNLOCK_ACTION.equals(action)) {
                  if (context.checkCallingPermission("android.permission.CONTROL_KEYGUARD") == android.content.pm.PackageManager.PERMISSION_GRANTED)
                      hideLocked();
              } else if (ALIYUN_KEYGUARD_GOTO_LOCKSCREEN_ACTION.equals(action)) {
                  if (mStatusBarKeyguardViewManager != null)
                      mHandler.post(new Runnable() {
                          public void run() {
                              mStatusBarKeyguardViewManager.goToLockScreen();
                          }
                      });
              } else if (ALIYUN_KEYGUARD_GOTO_UNLOCKSCREEN_ACTION.equals(action)) {
                  final boolean unlock = intent.getBooleanExtra("unlock", false);
                  final PendingIntent pending = intent.getParcelableExtra("pending");
                  if (mStatusBarKeyguardViewManager != null)
                      mHandler.post(new Runnable() {
                          public void run() {
                              mStatusBarKeyguardViewManager.goToUnlockScreen(unlock, pending);
                          }
                      });
              }
          }
  ...
  /*YUNOS END*/
    private RecentTasksLoader recentTasksLoader;
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            /// M: IPO shut down notify
                ...
                mIsIPOShutDown = true ;
                recentTasksLoader = RecentTasksLoader.getInstance(mContext);
                recentTasksLoader.loadTasksInBackground();
                mHandler.sendEmptyMessageDelayed(MSG_CLEAR_TASK, 500);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case ON_ACTIVITY_DRAWN:
                    ...
                    break;
                case MSG_CLEAR_TASK:
                    ArrayList<TaskDescription> mRecentTaskDescriptions = recentTasksLoader.getLoadedTasks();
                    int numRecentApps = mRecentTaskDescriptions != null ? mRecentTaskDescriptions
                            .size() : 0;
                    final ActivityManager am = (ActivityManager)
                            mContext.getSystemService(Context.ACTIVITY_SERVICE);
                    Log.w(TAG, "numRecentApps: " + numRecentApps + " mRecentTaskDescriptions = " + mRecentTaskDescriptions);
                    for (int i = 0; i < numRecentApps; i++) {
                        try {
                            am.removeTask(mRecentTaskDescriptions.get(i).getPersistentTaskId());
                            //recentTasksLoader.remove(mRecentTaskDescriptions.get(i));
                        } catch (Exception e) {
                        }
                    }
    ...
    private void sendUserPresentBroadcast() {
        synchronized (this) {
            if (mBootCompleted) {
                ...
                /*YUNOS BEGIN*/
              //##date:2014/09/19 ##kerong.skr ##BugID:5178106
              try {
                ...
                if (DEBUG) Log.e(TAG, "sendUserPresentBroadcast", e);
              }
              /*YUNOS END*/
            } else {
...
//            + "), mSuppressNextLockSound =" + mSuppressNextLockSound);
    ...
    private void playSound(int soundId) {
        ...
        if (Settings.System.getIntForUser(cr, Settings.System.LOCKSCREEN_SOUNDS_ENABLED, 1, UserHandle.USER_CURRENT) == 1) {
            /*YUNOS BEGIN*/
            //##modules(Keyguard): [disable locked sound when fancy lockscreen disabled]
            //##date:2014/08/14 ##kerong.skr
                        if (!isSecure() && AliKeyguardUtils.isFancyDisabled(mContext))
                            return;
            /*YUNOS END*/
    ...
    private void updateActivityLockScreenState() {
        /*YUNOS BEGIN*/
        //##modules(Keyguard): [directly goto unlock mode when show from external]
        //##date:2014/03/21 ##kerong.skr
        // Log.d(TAG, "updateActivityLockScreenState mShowing = "+mShowing+", mOccluded = "+mOccluded);
                if(mOccluded)
                    mStatusBarKeyguardViewManager.goToUnlockScreen(false, null);
        /*YUNOS END*/
        try {
    ...
    public static final int MSG_CLEAR_TASK = 1003;
